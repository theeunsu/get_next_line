...
/*   Updated: 2024/03/27 15:35:49 by eahn             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line_bonus.h"

// MAX_FD is defined in the header file as 10240
// the number was found using 'ulimit -a'
// fd number cannot be modifed more than 10240

char	*get_next_line(int fd)
{
	char		*line;
	static char	*static_str[MAX_FD];
	int			newline_len;

	if (fd < 0 || BUFFER_SIZE <= 0 || fd >= MAX_FD)
		return (NULL);
	static_str[fd] = read_save_str(fd, static_str[fd]);
	if (!static_str[fd])
		return (NULL);
	line = extract_line(static_str[fd], &newline_len);
	static_str[fd] = extract_next_str(static_str[fd], &newline_len);
	return (line);
}
